 ...
 * ╚═══════════════════════════════════════════════════════════════════════╝
 * 
 * @file           ch375_uart_stm32.c
 * @brief          CH375 UART hardware interface implementation (STM32)
 * 
 * @author         destrocore
 * @date           2025
 * 
...
/**
  ...
  */
int ch375_stm32_hw_init(const char *name, int usart_index, const struct gpio_dt_spec *int_gpio, uint32_t initial_baudrate, struct ch375_Context_t **ppCtxOut) {
    ...
    if (CH375_A_USART_INDEX != usart_index && CH375_B_USART_INDEX != usart_index) {
        LOG_ERR("Invalid USART index: %d", usart_index);
        return -EINVAL;
    }
    ...
    // Initialize GPIO and clocks for USART2
    if (CH375_A_USART_INDEX == usart_index && !usart2_init_done) {
        ...
        LOG_INF("Initialized clock/pins for USART2");
    }
    // Initialize GPIO and clocks for USART3
    else if (CH375_B_USART_INDEX == usart_index && !usart3_init_done) {
        ...
        LOG_INF("Initialized clock/pins for USART3");
    }

    /* Allocate hardware context */
    hw = k_malloc(sizeof(ch375_HwContext_t));
    if (NULL == hw) {
        LOG_ERR("Failed to allocate ch375_HwContext_t");
        return -ENOMEM;
    }
    ...
    if (ret < 0) {
        LOG_ERR("%s: Failed to configure 9-bit mode: %d", name, ret);
        k_free(hw);
        return ret;
    }
    ...
    *ppCtxOut = pCtx;
    LOG_INF("%s: STM32F4 hardware initialized (USART%d)", name, usart_index);
...
/**
  ...
  * @retval 0 on success, error code otherwise
  */
int ch375_stm32_set_baudrate(struct ch375_Context_t *pCtx, uint32_t baudrate)
{
...
static int ch375_read_data_cb(struct ch375_Context_t *pCtx, uint8_t *pData)
{
    ...
    if (ret < 0) {
        return (ret == -ETIMEDOUT) ? CH375_TIMEOUT : CH375_ERROR;
    }
